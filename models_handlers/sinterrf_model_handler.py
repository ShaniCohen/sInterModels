...
from models_handlers.model_handlers_utils import calc_binary_classification_metrics_using_y_score,\
    ...
    calc_multiclass_classification_scores, get_stratified_cv_folds, get_predictions_df, split_cv_data
...
class kRFModelHandler(object):
    ...
    """
    def __init__(self):
        super(kRFModelHandler, self).__init__()
    ...
    def get_const_hyperparams(**kwargs) -> Dict[str, object]:
        ...
        logger.debug("getting const kRandomForestClassifier hyperparams")
    ...
    def get_model_args(seed: int = 22, verbose: int = 0, **kwargs) -> Dict[str, object]:
        """   sklearn.__version__ = '0.24.0'  """
        logger.debug("getting kRandomForestClassifier model_args")
        model_args = {
            # 1 - constructor args
            'random_state': seed,
            'verbose': verbose
    ...
    @staticmethod
    def eval_dataset(trained_model: RandomForestClassifier, X: pd.DataFrame, y: list, is_binary: bool, num_classes: int,
    ...
    def predict_on_folds(self, model_args: dict, cv_data: Dict[int, Dict[str, object]], **kwargs) -> \
            (Dict[object, Dict[str, Dict[str, float]]], Dict[int, pd.DataFrame], Dict[object, dict], Dict[object, dict],
             Dict[object, dict]):
        """
        Returns
        -------
    ...
    def run_loco(self, model_args: dict, feature_cols: List[str], **kwargs) -> Dict[str, Dict[str, object]]:
        ...
        loco_shap_args - Dict in the following format:  {
            <fold>: {
                'model_nm': str
                'trained_model': <MODEL>
                "X_train": pd.DataFrame (n_samples, N_features),
        ...
        """
        logger.debug(f"running LOCO (w shap args)")
    ...
    def run_additional_cross_validation(self, model_args: dict, feature_cols: List[str], **kwargs) -> \
        ...
        """
        logger.debug(f"running kRandonForest additional cross validation")
        cv_outs = {}
    ...
    def train_and_test(self, X_train: pd.DataFrame, y_train: List[int], X_test: pd.DataFrame, y_test: List[int],
        ...
        shap_args - Dict in the following format:  {
            ...
            "X_test": pd.DataFrame,
            "y_train": List[int],
            "y_val": Optional[List[int]],
            "y_test": List[int],
            "X_metadata": Optional[pd.DataFrame],
            "y_metadata": Optional[pd.DataFrame]
        }
        """
        logger.debug("training a kRandomForest classifier")
        ...
        # 3 - train
        # todo - early stopping? other params?
    ...
    def train_and_test_w_negatives(self, train_df: pd.DataFrame, train_label_col: str,
        ...
        """
        logger.debug(f"training kRandomForest - with random negatives {train_set_nm}")
